...
class NVDiffRenderer(torch.nn.Module):
    ...
    def render_from_camera(
        ...
        if self.use_opengl:
            ...
            return self.render_mesh(verts, faces, RT, full_proj, image_size, background_color, face_colors=face_colors, attr=attr)
        else:
            ...
            output = self.render_mesh(verts, faces, RT, full_proj, image_size, background_color, face_colors=face_colors, attr=attr)
            for k, v in output.items():
                output[k] = F.interpolate(v.permute(0, 3, 1, 2), (cam.image_height, cam.image_width), mode='bilinear').permute(0, 2, 3, 1) if v is not None else None
            return output
    ...
    def render_mesh(
        ...
        rast_in_li = [z]
        if attr is not None:
            attr = attr.unsqueeze(0).unsqueeze(-1)
            rast_in_li.append(attr)
        rast_in = torch.cat(rast_in_li, dim=-1)
        rast_attr, _ = dr.interpolate(rast_in, rast_out, tri)
        ...
        if attr is not None:
            rast_attr = rast_attr[:, :, :, 1].unsqueeze(-1)
            rast_attr_filtered = torch.where(fg_mask, rast_attr, torch.tensor(0.0).cuda())

            # Convert to numpy for OpenCV colormap application
            rast_attr_filtered_np = rast_attr_filtered.detach().cpu().squeeze().numpy()
            rast_attr_filtered_np_uint8 = np.uint8(rast_attr_filtered_np * 255)  # Normalize to 0-255 for colormap
            rast_attr_colormap = cv.applyColorMap(rast_attr_filtered_np_uint8, cv.COLORMAP_TURBO)
            rast_attr_colormap_rgba = cv.cvtColor(rast_attr_colormap, cv.COLOR_RGB2RGBA)

            # Convert back to torch tensor
            rast_attr_rgba = torch.tensor(rast_attr_colormap_rgba, dtype=torch.float32).cuda().unsqueeze(0)
            rgba_attr = torch.where(fg_mask, rast_attr_rgba, rgba_bg)

        '''if attr is not None:
            cmap = plt.get_cmap('coolwarm')
            rast_attr = rast_attr[:, :, :, 1].unsqueeze(-1)
            rast_attr_min = rast_attr[fg_mask].min()
            rast_attr_max = rast_attr[fg_mask].max()
            rast_attr_normalized = (rast_attr - rast_attr_min) / (rast_attr_max - rast_attr_min) if rast_attr_max != rast_attr_min else rast_attr - rast_attr_min
            rast_attr_filtered = torch.where(fg_mask, rast_attr_normalized, torch.tensor(0.0).cuda())
            rast_attr_colormap = cmap(rast_attr_filtered.detach().cpu().squeeze().numpy())
            rast_attr_rgba = torch.tensor(rast_attr_colormap, dtype=torch.float32).cuda().unsqueeze(0)
            rgba_attr = torch.where(fg_mask, rast_attr_rgba, rgba_bg)'''
        ...
        return {
            ...
            'rgba': rgba_aa.flip(1),
            'rgba_attr': rgba_attr.flip(1) if attr is not None else None,
        }
