...
FLAME_PARTS_PATH = "flame_model/assets/flame/FLAME_masks_mouth.pkl" # FLAME Vertex Masks
FLAME_IGNORE_FACES_PATH = "flame_model/assets/flame/lower_neck_bottom_face_idcs.npy"  # faces to ignore when upsampling
...
class FlameHead(nn.Module):
    ...
    def __init__(
        ...
        flame_ignore_faces = FLAME_IGNORE_FACES_PATH,
        subdivision = 1,
        subdivision_mouth = 0,
        add_teeth = True
    ):
        ...
        self.register_buffer(
            "_faces", torch.from_numpy(np.load("flame_model/assets/flame/flame_faces.npy")).long(), persistent=False
        )
        self.register_buffer(
            "_vertex_uvs", torch.from_numpy(np.load("flame_model/assets/flame/vertex_uvs.npy")), persistent=False
        )
        self.register_buffer(
            "_faces_uvs", torch.from_numpy(np.load("flame_model/assets/flame/faces_uvs.npy")).long(), persistent=False
        )
        if add_teeth:
        ...
        with open(flame_parts_path, "rb") as f:
            ...
            self._parts["backneck"] = torch.from_numpy(np.load("flame_model/assets/flame/backneck_idcs.npy")).long()
            self._parts["shrink_scalp"] = torch.from_numpy(np.load("flame_model/assets/flame/shrink_scalp_idcs.npy")).long()
        ...
        # upsample
        self._ignore_faces = np.load(flame_ignore_faces)
        upsample_regions = dict(all=subdivision, mouth=subdivision_mouth)
        ...
        adjacent_faces = -torch.ones(self.faces.shape[0], 3, dtype=torch.long)
        roll_num = -torch.ones(self.faces.shape[0], 3, dtype=torch.long)
        if_flip = torch.zeros(self.faces.shape[0], 3, dtype=torch.bool)

        edge_to_face = -torch.ones(self._v_template.shape[0], self._v_template.shape[0], 2, dtype=torch.long)

        for face_idx, face in enumerate(self.faces):
            for i in range(3):
                edge_x = torch.min(face[i], face[(i + 1) % 3])
                edge_y = torch.max(face[i], face[(i + 1) % 3])
                
                if edge_to_face[edge_x, edge_y, 0] != -1:
                    edge_to_face[edge_x, edge_y, 1] = face_idx
                else:
                    edge_to_face[edge_x, edge_y, 0] = face_idx
        ...
        for face_idx, face in enumerate(self.faces):
            for i in range(3):
                ...
                if edge_to_face[edge_x, edge_y, 0] == face_idx:
                    a_face = edge_to_face[edge_x, edge_y, 1]
                else:
                    a_face = edge_to_face[edge_x, edge_y, 0]
                
                adjacent_faces[face_idx, (i + 2) % 3] = a_face

                if a_face != -1:
                    for j in range(3):
                        if self.faces[a_face, j] == face[i] and self.faces[a_face, (j + 1) % 3] == face[(i + 1) % 3]:
                            if i == j:
                                roll_num[face_idx, (i + 2) % 3] = 0
                            elif (i + 1) % 3 == j:
                                roll_num[face_idx, (i + 2) % 3] = 1
                            else:
                                roll_num[face_idx, (i + 2) % 3] = 2
                            break
                        elif self.faces[a_face, j] == face[i] and self.faces[a_face, (j - 1) % 3] == face[(i + 1) % 3]:
                            if_flip[face_idx, (i + 2) % 3] = True
                            if i == j == 1 or (i+1)%3 == (j-1)%3 == 1 or (i+2)%3 == (j-2)%3 == 1:
                                roll_num[face_idx, (i + 2) % 3] = 0
                            elif i == j == 2 or (i+1)%3 == (j-1)%3 == 2 or (i+2)%3 == (j-2)%3 == 2:
                                roll_num[face_idx, (i + 2) % 3] = 2
                            else:
                                roll_num[face_idx, (i + 2) % 3] = 1
                            break
        self.adjacent_faces = adjacent_faces.cuda()
        self.roll_num = roll_num.cuda()
        self.if_flip = if_flip.cuda()
