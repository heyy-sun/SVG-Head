...
from utils.log_utils import save_uv_map
import os
...
class Config:
    ...
    disable_flame_static_offset: bool = False
    """Disable the static offset of the FLAME model."""
    not_finetune_flame_params: bool = True
    """Disable the finetuning of the FLAME parameters."""
    not_finetune_xyz: bool = False
    """Disable the finetuning of the boundary conditions."""
    use_pts: bool = True
    """Enable point-based hybrid rendering."""
...
class GaussianSplattingViewer:
    ...
    def init_gaussians(self):
        ...
        self.gaussians = ShellGaussianModel(self.cfg.sh_degree, self.cfg.disable_flame_static_offset, self.cfg.not_finetune_flame_params, self.cfg.not_finetune_xyz)
        ...
        if self.cfg.point_path is not None:
            if self.cfg.point_path.exists():
                self.gaussians.load_params(self.cfg.point_path, has_target=False, motion_path=self.cfg.motion_path, disable_fid=unselected_fid)
            else:
    ...
    def define_gui(self):
        ...
        # window: edit ==================================================================================================
        with dpg.window(label="edit", tag="_edit_window", autosize=True):
            def callback_save_t_dc(sender, app_data):
                if not self.cfg.save_folder.exists():
                    self.cfg.save_folder.mkdir(parents=True)
                save_uv_map(self.gaussians.get_textures_dc, os.path.join(self.cfg.save_folder, "t_dc"), rgb = True)
            with dpg.group(horizontal=True):
                dpg.add_button(label="save t_dc texture", tag="_button_save_t_dc", callback=callback_save_t_dc)
            
            def callback_change_t_dc(sender, app_data):
                file_path = dpg.get_value("_new_t_dc_path")
                if os.path.exists(file_path):
                    ...
                    current_texture = self.gaussians._textures_dc.squeeze(0).permute(1, 2, 0).cpu().detach().numpy()
                    ...
                    self.gaussians.new_textures_dc = nn.Parameter(
            ...
            dpg.add_input_text(default_value="new t_dc path", tag="_new_t_dc_path")
            dpg.add_button(label="upload new t_dc path", tag="_button_upload_new_t_dc", callback=callback_change_t_dc)
        ...
        while dpg.is_dearpygui_running():
            ...
            if self.need_update or self.playing:
                ...
                if self.show_spatting:
                    ...
                    # rgb_splatting = render_func(cam, self.gaussians, self.cfg.pipeline, background_color, scaling_modifier=self.scaling_modifier, override_color=override_color)["render"].permute(1, 2, 0).contiguous()
