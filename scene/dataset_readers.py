...
class CameraInfo(NamedTuple):   # 相机信息
    uid: int
    R: np.array
    T: np.array
    FovY: np.array
...
class SceneInfo(NamedTuple):    # 场景信息
    train_cameras: list
    test_cameras: list
...
def getNerfppNorm(cam_info):    # 计算 NeRF++ 归一化参数
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):   # 从 Colmap 数据集中读取相机信息
    cam_infos = []
...
def storePly(path, xyz, rgb):   # 存储 ply 文件
...
def readColmapSceneInfo(path, images, eval, llffhold=8):    # 读取场景信息
    try:
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png"):
    ...
    train_cam_infos = readCamerasFromTransforms(path, "transforms_train_bak.json", white_background, extension)
...
def readDynamicNerfInfo(path, white_background, eval, extension=".png", target_path=""):    # TODO：改所有帧/单帧
    print("Reading Training Transforms")
    if target_path != "":
        train_cam_infos = readCamerasFromTransforms(target_path, "transforms_train_bak.json", white_background, extension)
    else:
        train_cam_infos = readCamerasFromTransforms(path, "transforms_train_bak.json", white_background, extension)
    
    print("Reading Training Meshes")
    train_mesh_infos = readMeshesFromTransforms(path, "transforms_train_bak.json")
    if target_path != "":
        print("Reading Target Meshes (Training Division)")
        tgt_train_mesh_infos = readMeshesFromTransforms(target_path, "transforms_train_bak.json")
    else:
